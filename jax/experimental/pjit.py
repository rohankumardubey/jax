...
def _check_resources_against_named_axes(what, aval, pos_axis_resources, named_axis_resources):
  ...
  if overlap:
    # TODO(yashkatariya, apaszke): Dynamically insert the original sharding as
    # an attribute on OpShardingSharding so that we can check the user spec
    # here.
    raise JAXTypeError(
        f"{what} has an axis resources specification of "
        f"{_get_single_pspec(pos_axis_resources.partitions)} "
...
def _resource_typing_pjit(avals, params, source_info, resource_env, named_axis_resources):
  ...
  for aval, s in zip(jaxpr.in_avals, params['in_shardings']):
    parsed_pspec = parse_flatten_op_sharding(s._op_sharding, resource_env.physical_mesh)[0]
...
def _sharding_constraint_mhlo_lowering(ctx, x_node, *, sharding,
  ...
  if hasattr(axis_ctx, "manual_axes") and axis_ctx.manual_axes:
    mesh = resource_env.physical_mesh
...
# TODO(yashkatariya): Don't cache the OpShardingSharding object directly. Cache
# the op sharding protos so that we get the same object id.
@lru_cache(maxsize=4096)
def to_op_sharding_sharding(s, ndim):
  return OpShardingSharding(s._device_assignment(), s._to_xla_op_sharding(ndim))
...
def _maybe_replace_from_gda_with_pspec(
  ...
  def _gda_check_and_get_sharding(gda_sharding, in_sharding, ndim):
    if not _is_from_gda(in_sharding) and not pxla.are_op_shardings_equal(
      ...
      # Convert to pspec as this code is taken by GDA which requires a Mesh
      # anyways.
      parsed_pspec = parse_flatten_op_sharding(in_sharding._op_sharding, gda_sharding.mesh)[0]
      pspec = _get_single_pspec(parsed_pspec)
      raise ValueError(
